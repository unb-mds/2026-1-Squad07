...
Validar, antes da aprovação da implementação, que o frontend consome somente contratos persistidos do backend e comunica corretamente estados relevantes para a demonstração da R1.

## Estratégia

- Preservar e executar os testes `pytest` existentes, que validam os contratos consumidos de autenticação e leis.
- Usar lint e build do Next.js para verificar tipos, imports e ausência de dependência dos mocks removidos.
