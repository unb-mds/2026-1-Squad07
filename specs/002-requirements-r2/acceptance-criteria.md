...
Cada bloco de critério de aceite abaixo está vinculado a um Épico correspondente do nosso User Story Mapping (USM). Os limites quantitativos de qualidade e performance de dados foram extraídos e validados em conformidade com o documento de Métricas de Sucesso da IA (`ai-success-metrics.md`).
...
#### AC 1.1 — Formato de Resposta Estruturada do Agente
* **GIVEN** que o Agente de IA proprietário finalizou o processamento de análise de uma lei submetida.
* **WHEN** o payload de resposta for retornado para a camada de Back-end (FastAPI).
* **THEN** o resultado deve obrigatoriamente se estruturar em formato JSON válido contendo os campos: `score` (inteiro de 0 a 100), `readability_index` (float), `summary` (string de até 500 caracteres) e um array de `ambiguities` contendo o segmento textual e a respectiva sugestão de melhoria.
...
#### AC 1.3 — Validação de Input Mínimo e Trava do Score
* **GIVEN** que o usuário submeteu uma proposição legislativa para análise.
* **WHEN** o texto fornecido for considerado inválido ou abaixo do tamanho mínimo operacional de 100 caracteres.
* **THEN** o sistema deve bloquear a chamada ao Agente de IA, retornar um código HTTP 400 (Bad Request) e exibir uma validação em tela informando que o texto não possui os requisitos mínimos para o cálculo do Score de Qualidade.
...
#### AC 3.3 — Vinculação Interativa de Destaques e Cards de Ambiguidade
...
* **THEN** a interface deve aplicar um destaque visual reflexo no card lateral correspondente àquela ambiguidade e trazê-lo para o foco de visão do usuário (*scroll into view*).
...
| TS-AC02 | Testabilidade de Cláusulas | Confirmação de que todas as condições `THEN` possuem métricas exatas e testáveis (ex: 85% precisão, JSON com campos estritos, latência, expiração de 15 min) para guiar a codificação do `test-plan.md`. | Aprovado |
| TS-AC03 | Tratamento de Desvio de Escopo | Validação se o fluxo de autenticação avançada contém ressalva explícita de cronograma, evitando conflitos de entrega com o PO. | Aprovado |